...
Infra notes: `RandomForestJudge` / `derive_threshold` need the `[trained]` (scikit-learn) extra. The DSPy consumer **already exists in-repo** — `DSPyJudge` (`core/modules/dspy_judge.py`, `@register("dspy_judge")`) exposes `compile(optimizer="bootstrap"|"mipro")` wrapping `dspy.BootstrapFewShot` / `dspy.MIPROv2` (`dspy_judge.py:297-312`), wired via `methods.py:219`. So the hard-demo pre-filtering in §7 is **testable against the existing seam**, not a from-scratch build.
