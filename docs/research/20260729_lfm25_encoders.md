...
**Benchmarks that do separate the two, but narrowly (&lt;0.05 of range): `walmart_amazon` (control below the paired baseline).** This is a statement about **resolution, not significance**. Each of those intervals clears zero against `LiquidAI/LFM2.5-Embedding-350M` — the model it is computed against, and the only one this column can speak for — in the direction marked. That separation is real. But the whole trained-vs-random range is small, so a reported margin can be a large fraction of it, and a ranking built from differences of that size is fragile rather than meaningless.
