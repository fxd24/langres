...
| [`20260728_prompt_axis_findings.md`](20260728_prompt_axis_findings.md) ⭐ | results | **Do better instructions make instruction-following embedders better at blocking, and can we pick good ones rather than guessing?** Five checkpoints × four benchmarks × up to seven prompt recipes, reported **per model × per benchmark and never averaged**, on candidate recall at fixed `k` (never F1 at a threshold). Every documented prompt format is quoted from the checkpoint's *own* `config_sentence_transformers.json` or model card at a **pinned revision** — none inferred from another model's convention, since they have nothing in common. **The answer is three tiers, not two:** the checkpoint's own *retrieval* prompt is a reliable win (significantly positive on ≥1 benchmark for **three of the four** instruction-trained models, never significantly negative anywhere); substituting your own task text into its template shape is a **coin flip** (helped Qwen3 +0.0388, cost Gemma −0.1589) and never beat the model's own default on recall; a raw English sentence outside any template **significantly harmed all 5** checkpoints somewhere, and EmbeddingGemma is the only one it also *helped* somewhere (+0.0118 on `amazon_google`) — that positive cell is kept in the table rather than dropped, and so is Gemma's own −0.0078, which an earlier "Gemma is the exception" framing had omitted. **"Documented" is not sufficient, which is the trap:** Gemma's `clustering` template is exactly as official as its retrieval one and costs **−0.3183** Δ per-record recall. Two model-card predictions were tested: Qwen3's "1% to 5%" held, and e5's — `query:` on both sides for symmetric tasks, which ER blocking looks exactly like — bought **no measurable improvement**. Every *comparison* is **corrected for multiplicity** (Holm, one `(model, arm)` per family) — the correction applies to the bootstrap p-values and the significance verdicts, in their own column; the intervals stay plain 95% intervals and are not simultaneous. That correction is what removes e5 from tier 1: both its **documented** families fail at Holm's first step. The study's retraction list is **five** cells — e5's three, plus two of bge's that a normal-approximation p-value had been hiding; recomputing the p-values from the bootstrap replicates themselves is what exposed them, and bge's headline `official_query_instruction` is untouched. That is *not* "prompts do nothing to e5" — the axis moves this checkpoint hard and only ever downward: its `er_symmetric` arm (our own ER sentence, both sides) costs it −0.1038 on `wdc_computers` and survives the correction comfortably — the **worst any checkpoint fares under that shared sentence** (not the largest harm in the sweep, which is Gemma's documented `clustering` template at −0.3183). e5's two documented recipes buy nothing; a plausible hand-written one costs a tenth of its recall. Carries its **own** corrections: an interval that closes on zero is reported as a boundary case, not a win; the multiplicity p-values are the bootstrap's own achieved significance level over **20 000** replicates, not a normal tail recovered from an interval endpoint (which is uncalibrated at every level Holm actually tests at); the model-specificity claim was rebuilt on `er_symmetric` (the one genuinely cross-model-identical arm) after the arm originally cited turned out to differ per checkpoint by design; two cited snapshot hashes were wrong because the harness loaded by mutable repo name; and an arm-vs-arm argument resting on non-overlapping intervals was withdrawn as never having been a test. Generated by `examples/research/prompt_axis.py`; rows carry the checkpoint revision and a prompt fingerprint, so a re-run cannot silently mix studies. |
...
| [`20260729_lfm25_encoders.md`](20260729_lfm25_encoders.md) | results | **LiquidAI's LFM2.5 encoders on ER candidate blocking** — `LFM2.5-Embedding-350M` against `intfloat/e5-base-v2` (the *current* `DEFAULT_EMBEDDING_MODEL`, and this study's baseline, unlike the 2026-07-27 ladder whose baseline is the older default), plus the two base masked-LM encoders in a **separate, clearly-labelled artifact** because they ship no pooling config and sentence-transformers gives them an *untrained* mean-pooling head — not a like-for-like comparison, and their low scores say nothing about the backbone. Generated end-to-end by `examples/research/lfm25_report.py` from two tracked JSONLs plus `20260729_lfm25_load_probe.json`; **no measured number is typed into the prose.** **Result: `LFM2.5-Embedding-350M` is ahead of `e5-base-v2` on no benchmark/arm**, and behind with an interval excluding zero on the `none` and `instruct` arms of `abt_buy`, `amazon_google` and `wdc_computers`. Read that as **per-arm, not checkpoint-level**: a paired interval needs both models' per-record vectors, so the checkpoint's own `documented`-prompt arm — which on `abt_buy` scores **0.9741** against e5's best **0.9703** — has no counterpart in the baseline and is untestable here rather than a loss. Three things to read before quoting it. (1) **The licence is a shipped-default blocker**: all three are LFM Open License v1.0, whose §5 conditions Commercial Use on not exceeding a **$10M annual-revenue Threshold** — verified against the byte-identical `LICENSE` committed here as `20260729_lfm25_license.txt`, not against the card summary. Opt-in named model, never `DEFAULT_EMBEDDING_MODEL`; langres is Apache-2.0. (2) **A random-init noise-floor control** (`random-init-control-350M`, seeded, same architecture, no training) runs on every benchmark, and the gap above it is the **observed gap** between the best tuned model measured and **one seeded random initialisation** — not the benchmark's dynamic range and not a bound, since control variance was never measured (one seed, no replicates). Measured: **one of five benchmarks cannot support an embedder claim** — `fodors_zagat` (margin `+0.0000`, reclassifying it from merely *saturated* to **uninformative in the strict sense**). `walmart_amazon` *does* separate trained from random (the control's paired intervals `[-0.0275, -0.0072]` and `[-0.0295, -0.0099]` both exclude zero), but the whole observed gap from that random control to the best tuned model is only `+0.0183`, so a margin there is a large fraction of that one observed gap — a **resolution** caveat, not a significance one, and the gap is an observation rather than a bound (a different seed or a stronger model moves either endpoint). An earlier revision of both documents called it uninformative on the strength of that `+0.0183` point gap; that was a point estimate used as a variance estimate, and the measured intervals in the same file contradict it. The control also **matches or beats both real base encoders on 5 of 5** benchmarks — a result about untrained pooling rather than about those checkpoints; the *causal* reading is claimed only for the matched-backbone pair (`Encoder-350M` vs the 350M control), since the 230M encoder varies backbone and size as well as pretraining. (3) Intervals are paired per-record, resampled **by gold cluster**, **nominal and uncorrected** across benchmarks — the doc states the family-wise rate rather than implying a guarantee, and flags any bootstrap bound landing exactly on `0.0000` as *not* excluding zero. Companion: `20260729_lfm25_tuned*` and `20260729_lfm25_base_encoders*`. |
