...
| [`20260716_import_graph_baseline.md`](20260716_import_graph_baseline.md) | results/audit | **Import-graph baseline** for the architecture refactor — re-derives the fan table, the kinds/SCC split, and the cross-package cycle counterfactual with the committed `tools/import_graph.py`. Mostly a **negative result**: the "14 vs 6 cycles" claim does not reproduce, and 21% of edges are deliberately-lazy ones import-linter cannot ignore. |
