...
------------------------------------------------------------------
``RFJudge.fit`` (the demo's student) raises an opaque scikit-learn error if it
...
def _assert_both_classes_in_seeded_split(candidate_pairs: list[dict[str, Any]]) -> None:
    """Guard the RFJudge single-class trap: the demo's train half must span both classes.
    ...
    if train_labels != {True, False}:
        raise AssertionError(
            ...
            "RFJudge.fit would fail on single-class labels -- adjust the scenario "
