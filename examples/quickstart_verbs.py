...
Offline on purpose: this example pins matcher="string", so no API key is
...
The default matcher="auto" is different: it requires an LLM API key
(OPENROUTER_API_KEY/OPENAI_API_KEY, plus the [llm] extra) and raises
NoMatcherAvailableError without one -- langres never silently falls back to
...
if os.environ.get("OPENROUTER_API_KEY") or os.environ.get("OPENAI_API_KEY"):
    print(
        ...
        'default matcher="auto" would pick a real LLM judge instead of the '
        "zero-spend 'string' one. This example pins matcher=\"string\" deliberately "
        'to stay free and fully offline -- drop that kwarg (or pass matcher="auto") '
        "to try it, spend-capped at $1 by default (budget_usd=)."
    )
else:
    print(
        "\nNo OPENROUTER_API_KEY/OPENAI_API_KEY set: dedupe(records) with the "
        'default matcher="auto" would raise NoMatcherAvailableError -- langres '
        "never silently falls back to fuzzy matching. This example opts into "
        "the zero-spend 'string' judge explicitly (matcher=\"string\"). To use a "
