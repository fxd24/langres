...
via OpenRouter) bootstraps silver labels; a cheap RandomForestMatcher student is trained; a
...
#: Floor on positives so both the RandomForestMatcher train half and the held-out metric split
#: reliably span both label classes (all-True labels crash ``RandomForestMatcher.fit``).
_MIN_POSITIVES = 30
