...
# The renderer re-checks its own inputs at render time -- it is the only thing
# that knows all of them, and the study-A resume reaches it by a different path.
# This block used to live here, which left that second caller unguarded; moved
# into lfm25_report.py rather than copied into both. Its refusal is a non-zero
# exit, so it lands in the same close-and-commit branch below as any other render
# failure. (Cross-model review.)
#
