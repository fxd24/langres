"""The concrete rapidfuzz-backed Comparator: :class:`StringComparator`.

Split out of ``langres/core/comparator.py`` in W1 so the ``Comparator``
**contract** and this **implementation** stop sharing a module. They are
different layers: the ABC is what a pipeline is written against, this is one
way to satisfy it. While they shared a file, every module that imported the
contract also imported rapidfuzz and the impl -- and the refactor's target
layout had to map the whole file to ``components``, which made the *contract*
sit above the components that depend on it. That single mis-layered module was
the only thing keeping ``components <-> core`` a two-way knot; see
``tests/test_import_tangle.py`` for the measurement.

Import it from the package that owns it, alongside its blocker/matcher peers::
