...
the run schema (:mod:`langres.tracking.runs`) and *pushes* it into whatever backend
...
so ``from langres.tracking.trackers import MlflowTracker`` works, but importing this
...
_ADAPTERS: dict[str, tuple[str, str]] = {
    "mlflow": ("langres.tracking.trackers.mlflow_tracker", "MlflowTracker"),
    "wandb": ("langres.tracking.trackers.wandb_tracker", "WandbTracker"),
    "trackio": ("langres.tracking.trackers.trackio_tracker", "TrackioTracker"),
}
...
class ExperimentTracker(Protocol):
    ...
    the :class:`~langres.tracking.runs.RunContext` into params/tags the backend
    understands and streams metrics/artifacts as the run progresses.
    ``run_url`` deep-links back into the backend UI (threaded into
    :attr:`~langres.tracking.runs.RunRecord.artifacts`); ``native`` is the escape
