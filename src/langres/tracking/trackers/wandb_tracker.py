...
``from langres.tracking.trackers import WandbTracker`` -- it pulls ``wandb`` only
transitively (via :func:`langres.tracking.factories.create_wandb_tracker`), so it
...
:class:`~langres.tracking.runs.RunContext`.
"""
...
class WandbTracker:
    ...
    One :class:`~langres.tracking.runs.RunContext` becomes one W&B run: the context
    ...
    UI (threaded into :attr:`~langres.tracking.runs.RunRecord.artifacts`);
