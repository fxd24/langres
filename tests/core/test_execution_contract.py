...
from langres.core.op import ExecutionEvent, Score, ThresholdSelect
...
def test_observer_failure_is_isolated_explicit_and_cannot_change_results() -> None:
    def broken(_event: ExecutionEvent) -> None:
        raise LookupError("observer broke")
    ...
    assert {error.message for error in observed.observer_errors} == {"observer broke"}
