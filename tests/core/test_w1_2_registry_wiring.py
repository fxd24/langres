...
``"random_forest"`` raises ``UnknownComponentType``.
...
``tests/core/modules/test_random_forest_judge.py`` -- this file only covers the
...
def test_new_w1_2_components_register_via_langres_core_alone() -> None:
    """A fresh process resolves fellegi_sunter_judge/random_forest via
    ...
    result = subprocess.run(
        [
            ...
            "for name in ('fellegi_sunter_judge', 'random_forest'):\n"
            "    get_component(name)\n"
            "print('OK')\n",
        ],
        capture_output=True,
...
def test_lazy_component_modules_includes_new_w1_2_components() -> None:
    """fellegi_sunter_judge/random_forest resolve via the lazy registry path
    ...
    ``"fellegi_sunter_judge"``/``"random_forest"`` would raise
